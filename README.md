...
- one reusable Ubuntu-based image
- GUI tool support on the host desktop
- a fixed data mount inside the container
- helper scripts for non-expert users
- `yarpmanager` applications for the same tools and demos

On this machine, the current host dataset root is:

```text
/home/bmaacaron-iit.local/Documents/Datasets/2026_PRIMI/
```

Inside the container, that appears as:

```text
/workspace/data
```

So the BallBalance dataset is currently available in the container at:
...
This section is the minimum you need to operate the workstation in its different modes.

### Installation and Setup

Another user does need a few host-side tools installed.
...
Users do **not** need to install these on the host:
...
The current implemented behavior is:

- image build works through `./scripts/build.sh`
- workstation startup works through `./scripts/start-workstation.sh`
...
- project root: `/home/bmaacaron-iit.local/Documents/Dockers/YarpinatorDocker`
...
- `VFramer`
  - opens `vFramer` using the default source in [`yarpmanager/defaults.env`](yarpmanager/defaults.env)
- `All Tools`
...
- `BallBalance Moving Demo`
  ...
  - uses a coordinated launcher so the viewers do not race each other during startup
- `BallBalance Stationary Demo`
...
## Reference and Extra Details

This section is not required for normal day-to-day use, but it explains how the project is put together and how to work at a lower level.

### Core Concepts

`Docker image`
: the built environment defined by [Dockerfile](Dockerfile)

`Docker container`
: the running instance of that image

`docker compose`
: the runtime definition in [compose.yaml](compose.yaml)
...
The helper layer in [`scripts/common.sh`](scripts/common.sh) standardizes:
...
The container entrypoint in [`container-scripts/container-entrypoint.sh`](container-scripts/container-entrypoint.sh) remaps the internal `robotology` user to the current host user’s numeric `UID` and `GID` at container start. That helps avoid file ownership problems on bind-mounted folders.
...
If `vFramer` needs a different default source, edit [`yarpmanager/defaults.env`](yarpmanager/defaults.env):
