...
select action in "${actions[@]}"; do
    case "$REPLY" in
        ...
        4) "$SCRIPT_DIR/demo-ballbalance.sh" ;;
        5) "$SCRIPT_DIR/open-yarpview.sh" ;;
        6) "$SCRIPT_DIR/open-yarpscope.sh" ;;
        7) "$SCRIPT_DIR/open-vframer.sh" left ;;
        8) "$SCRIPT_DIR/open-vframer.sh" right ;;
        9) "$SCRIPT_DIR/open-skingui.sh" ;;
